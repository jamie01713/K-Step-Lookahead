...
def policy_names(learners, n_state, n_action, model_type):
    model = MD_10(n_states=n_state, n_actions=n_action, entropy=SeedSequence(0))
    return [policy(model, model_type=model_type, **cfg).name() for policy, cfg in learners]


def run_one_experiment(entropy, n_state, n_action, n_replications_per_experiment, n_horizon, learners, model_type):
    model = MD_10(n_states=n_state, n_actions=n_action, entropy=entropy[0])
    ...
    for run in range(n_replications_per_experiment):
        for policy in policies:
            ...
            results[policy.name()]["average_reward"][:, run] = np.asarray(info["average expected reward"])
    return results
...
def main(
    ...
    name_policies={
        ...
        LG1_2T_Adaptive: {"threshold": [0.9], "power": 1 / 2, "threshold_i": 0.3, "change_point": 10 * 5 * 20000},
    },
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name_policies", type=object, default={LG1T: {"threshold": 0.3}, LG2T: {"threshold": [0.9], "power": 1 / 2}, LG1_2T_Adaptive: {"threshold": [0.9], "power": 1 / 2, "threshold_i": 0.3, "change_point": 10 * 5 * 20000}})
